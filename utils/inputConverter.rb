...
(0..simduration).step(1) do |start_time|
  sources.keys.each { |source|
    ...
    #aut_random = sources[source].to_f / (60*60)
    #aut_temp = aut_random / (aut_random.ceil)
    #while aut_random > 1
      ...
      #aut_random -= aut_temp
        ...
        if rand.rand < autonomous[0]/100 # if autonomous
          f.write("  <vehicle depart=\"#{start_time}\" id=\"veh#{vehicle_id}\" route=\"#{routes[source][i][2]}\" type=\"CarA\" departSpeed=\"max\"/>\n");  
        else # if manual vehicle
          f.write("  <vehicle depart=\"#{start_time}\" id=\"veh#{vehicle_id}\" route=\"#{routes[source][i][2]}\" type=\"CarM\" departSpeed=\"max\"/>\n");  
        end
      end
    #end
  }
