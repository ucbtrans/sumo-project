    ...
    while traci.simulation.getMinExpectedNumber() > 0 and step <= run_time*(1/step_length): 
        ...
        if len(sensor_data) != 0:
            ...
            if sensor_data[0][3] != -1: #if the vehicle is leaving the sensor, record the time it left
                leaving_times.append(sensor_data[0][3])
            prev_veh_id = veh_id
...
if __name__ == "__main__":
    ...
    print tau_effective(30)
    for x in run_times:

    # this is the normal way of using traci. sumo is started as a
    # subprocess and then the python script connects and runs
        sumoProcess = subprocess.Popen([sumoBinary, "-c", "single_lane/hello.sumocfg","--step-length", str(step_length), "--remote-port", str(PORT)], stdout=sys.stdout, stderr=sys.stderr)
