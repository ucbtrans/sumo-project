...
def run(run_time):
    ...
    while traci.simulation.getMinExpectedNumber() > 0 and step <= run_time*(1/step_length): 
        ...
        if len(sensor_data) != 0:
            ...
            prev_veh_id = veh_id
        if step <= 600:
            traci.trafficlights.setRedYellowGreenState("13",NSGREEN)
        else:
            traci.trafficlights.setRedYellowGreenState("13", PROGRAM[programPointer])
        if step <= 4000:
            traci.trafficlights.setRedYellowGreenState("4","rrGGgrrrrrrr")
        else:
            traci.trafficlights.setRedYellowGreenState("4","GGrrrGGGGGGG")
